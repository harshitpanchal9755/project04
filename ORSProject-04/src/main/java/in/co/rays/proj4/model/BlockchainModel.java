...
public class BlockchainModel {
	...
	public Integer nextpk() throws DataBaseException {
        ...
        try {
            ...
            PreparedStatement ps = conn.prepareStatement("select max(id) from blockchain");
     ...
     */
    public long add(BlockchainBean bean) throws ApplicationException, DuplicateException {
    	

        Connection conn = null;
        int pk = 0;

        BlockchainBean existBean = findBypk(bean.getId());

        if (existBean != null) {
            throw new DuplicateException("blockcahin already exist");
        }
        ...
        try {
            ...
            ps.setInt(1, pk);
            ps.setString(2, bean.getTranscactionHash());
            ps.setString(3, bean.getSenderAddress());
            ps.setString(4, bean.getReceiverAddress());
            ps.setDouble(5, bean.getGasfee());
     ...
     * @throws DuplicateException   if a role with the same name already exists
     */
    public void update(BlockchainBean bean) throws ApplicationException, DuplicateException {
        ...
        try {
            ...
            PreparedStatement ps = conn.prepareStatement(
                    "update blockchain set transactionHash = ?, senderAddress = ?, receiverAddress, gasfee = ?, created_by = ?, modified_by = ?, created_datetime = ?, modified_datetime = ? where id = ?");

            ps.setString(1, bean.getTranscactionHash());
            ps.setString(2, bean.getSenderAddress());
            ps.setString(3, bean.getReceiverAddress());
            ps.setDouble(4, bean.getGasfee());
     ...
     */
    public void delete(BlockchainBean bean) throws ApplicationException, DuplicateException {
        ...
        try {
            conn = JdbcDataSource.getConnection();
            conn.setAutoCommit(false);
            PreparedStatement ps = conn.prepareStatement("delete from blockcahin where id = ?");
     ...
     */
    public BlockchainBean findBypk(long pk) throws ApplicationException {
        BlockchainBean bean = null;
        Connection conn = null;

        StringBuffer sql = new StringBuffer("select * from blockchain where id = ?");
        ...
        try {
            ...
            while (rs.next()) {
                bean = new BlockchainBean();
                bean.setId(rs.getLong(1));
                bean.setTranscactionHash(rs.getString(2));
                bean.setSenderAddress(rs.getString(3));
                bean.setReceiverAddress(rs.getString(4));
                bean.setGasfee(rs.getDouble(5));
    ...
    public List<BlockchainBean> list() throws ApplicationException {
     ...
     */
    public List<BlockchainBean> search(BlockchainBean bean, int pageNo, int pageSize) throws ApplicationException {
        StringBuffer sql = new StringBuffer("select * from blockchain where 1=1");

        if (bean != null) {
            if (bean.getId() > 0) {
                sql.append(" and id = " + bean.getId());
            }
            if (bean.getTranscactionHash() != null && bean.getTranscactionHash().length() > 0) {
                sql.append(" and transactionHash like '" + bean.getTranscactionHash() + "%'");
            }
            if (bean.getSenderAddress() != null && bean.getSenderAddress().length() > 0) {
                sql.append(" and senderAddress like '" + bean.getSenderAddress() + "%'");
            }
            if(bean.getReceiverAddress() != null && bean.getReceiverAddress().length() > 0) {
            	sql.append(" and receiverAddress like '" + bean.getReceiverAddress() + "%'");
            }
            if(bean.getGasfee() > 0) {
            	sql.append(" and gasfee = " + bean.getGasfee());
            }
        }
